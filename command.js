...
enemyChar.prototype.title = function () {
	...
	for(var i=0;i<enemyNumber;i++)
	{
		...
		enemyType[eRandom].HP(), enemyType[eRandom].movement(), eDistance[i],
		 enemyType[eRandom].tempImage(), 100 + gapX, 5);//MONSTER Y IS 5, MONSTER X IS 20 GAP
...
actionTurn = function(id,minus)
{
	...
	var actionTimer = setInterval(function(){eTurn()},125);
			...
			for(var i=0;i<enemyNumber;i++)
			{
				...
				if(playerHP > 0)
				{
					...
					if(enemyPosition[i] != null)
					{
						...
						tempPos = (enemyPosition[i].movement())*40;
						...
						tempDist[i] = enemyPosition[i].enemyDistance();
						
						tempY[i] = enemyPosition[i].enemyY();
						if(actionNumber == 7)
						{
							tempPos = tempPos*8;
							tempDist[i] = enemyPosition[i].enemyDistance() - enemyPosition[i].movement();
							tempY[i] = enemyPosition[i].enemyY()+(enemyPosition[i].movement())*40;
						...
						else if(actionNumber == 1)
						{
							...
							if(checkTest < enemyNumber + 1)
							{
								check[i] = tempPos + (7-enemyPosition[i].enemyDistance())*50;
							}
							else
							{
								check[i] = tempPos + check2[i];
							}
						...
						if(actionNumber == 7)
						{
							console.log('' + tempPos + ', ' + check[i] + ', ' + check2[i]);
						}
					
					enemyPosition[i] = new enemyPos(enemyPosition[i].title(), 
							enemyPosition[i].HP(), enemyPosition[i].movement(), 
							tempDist[i], enemyPosition[i].enemyImage(), enemyPosition[i].enemyX(), tempY[i]);
...
command.timingForBat = function (id, t, time)
{
	...
	function intTimer()
	{
		...
		{	a=true;
			...
			for(var i=0;i<enemyNumber;i++)
			{
				document.getElementById('aim'+ i).disabled = false;	
			}
			clearInterval(timer);
